      ...
      <CardContent>
        <div className="space-y-4">
          <div className="text-center">
            <div className="text-4xl font-bold text-sky-400 mb-2">{score}/100</div>
            <Badge variant={score > 80 ? "default" : "destructive"} className="bg-sky-600">
              {score > 80 ? "Strong Protection" : "Needs Improvement"}
            </Badge>
          </div>
          
          <div className="space-y-2">
            <h4 className="font-medium text-sky-300">Security Factors</h4>
            {improvements.map((item, index) => (
              <div key={index} className="flex items-center justify-between text-sm">
                <span className="text-sky-100/70">{item.item}</span>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-sky-200/60">{item.impact}</span>
                </div>
              </div>
            ))}
          </div>
